...
from .hid_sender import HIDSender, NullSender
...
def main():
    ...
    parser.add_argument("--screen-monitor", default="primary",
                         help="Which display to capture for --screen: 'primary' (default) or a 0-based index")
    parser.add_argument("--screen-fps", type=int, default=None,
                         help="Screen capture rate in Hz (default: backend-recommended)")
    ...
    try:
        ...
        if args.simulator:
            print(f"[!] No keyboard found ({e}); running with HID disabled (simulator only).")
            sender = NullSender()
        else:
            print(f"[-] Error: {e}")
            return
    ...
    if args.screen:
        ...
        display_index = 0 if args.screen_monitor == "primary" else int(args.screen_monitor)
        screen_analyzer = ScreenAnalyzer(display_index=display_index, target_fps=args.screen_fps)
    ...
    def signal_handler(sig, frame):
        # Just flag the loop to exit; let it unwind through the `with`
        # blocks below normally instead of tearing down HID/terminal state
        # from inside the handler (which could race a send_packet() in
    ...
    try:
                ...
                while not stop_event.is_set():
                    ...
                    if dt >= update_interval:
                        ...
                        if args.screen:
                            ...
                            current_p_name = screen_analyzer.status_label
                            ...
                            if p_rot:
                                ...
                                if features['kick'] > 0.5:
                                    hue_rotation += 3.0
                                hue_rotation = (hue_rotation + speed) % 256.0
                            else:
                                # Subdued "shimmer" for chic palettes
                                hue_rotation = (hue_rotation + 0.1) % 256.0

                            h_b = int((base_hues[0] + hue_rotation * 0.1) % 256)
                            h_m = int((base_hues[1] + hue_rotation * 0.5) % 256)
                            h_t = int((base_hues[2] + hue_rotation * 1.0) % 256)
        ...
        # Runs after the `with` blocks above have already unwound (stream
        # stopped, terminal restored), so this only tears down the pieces
        # that own no terminal/audio-device state of their own.
        if args.screen:
